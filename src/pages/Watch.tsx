...
const Watch: React.FC = () => {
    ...
    const GoToHomePageButton = () => {
        ...
        return (
            <StyledHomeButton onClick={handleClick}>
                Go back Home
            </StyledHomeButton>
        );
    };
    ...
    useEffect(() => {
        ...
        setLanguage(
            localStorage.getItem(getLanguageKey(animeId || "")) ||
                defaultLanguage,
        );
    }, [animeId]);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const animeData = await fetchAnimeEpisodes(
                    animeId,
                    undefined,
                    isDub,
                );
                if (isMounted && animeData) {
                    const transformedEpisodes = animeData
                        ...
                        .map((ep: any) => {
                            ...
                            const episodeNumberMatch =
                                episodePart.match(/^(\d+(?:-\d+)?)/);
                            return {
                                ...ep,
                                number: episodeNumberMatch
                    ...
                    const navigateToEpisode = (() => {
                        if (languageChanged) {
                            ...
                            const savedEpisodeData = localStorage.getItem(
                                LOCAL_STORAGE_KEYS.LAST_WATCHED_EPISODE +
                                    animeId,
                            );
                    ...
                    if (navigateToEpisode) {
                        ...
                        const newAnimeTitle =
                            navigateToEpisode.id.split("-episode-")[0];
                        navigate(
    ...
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            ...
            if (
                !event.shiftKey ||
                !["N", "P"].includes(event.key.toUpperCase())
            )
                return;
            const now = Date.now();
