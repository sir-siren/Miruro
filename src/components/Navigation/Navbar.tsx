...
import { fetchSearchFromBackend, type Anime } from "../..";
...
export const Navbar = () => {
    ...
    const fetchSearchResults = async (query: string) => {
        ...
        try {
            const fetchedData = await fetchSearchFromBackend(query);
            // Backend returns array of anime directly
            const formattedResults = fetchedData
                .slice(0, 5)
                .map((anime: any) => ({
                    id: anime.id,
                    title: anime.title,
                    image: anime.coverImage?.large || anime.coverImage,
