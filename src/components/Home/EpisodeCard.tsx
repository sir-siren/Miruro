...
const AnimeEpisodeCard = styled(Link)`
    ...
    &:focus {
        ...
        img {
            filter: brightness(
                0.5
            ); // Optional: Slightly darken the image itself
        }
    }
...
export const EpisodeCard: React.FC = () => {
    ...
    const episodesToRender = useMemo(() => {
        ...
        try {
            ...
            const lastEpisodes = Object.entries(
                allEpisodes,
            ).reduce<LastEpisodes>((acc, [animeId, episodes]) => {
            ...
            return orderedAnimeIds.map((animeId) => {
                ...
                const handleRemoveAllEpisodes = (animeId: string) => {
                    const updatedEpisodes = JSON.parse(
                        watchedEpisodesData || "{}",
                    );
                    delete updatedEpisodes[animeId];

                    const newWatchedEpisodesData =
                        JSON.stringify(updatedEpisodes);
                    localStorage.setItem(
                        "watched-episodes",
                        newWatchedEpisodesData,
                    );
                ...
                return (
                    <StyledSwiperSlide key={episode.id}>
                        ...
                        >
                            ...
                            <ProgressBar
                                style={{
                                    width: `${Math.max(playbackPercentage, 5)}%`,
                                }}
                            />
                            <CloseButton
                                onClick={(e) => {
